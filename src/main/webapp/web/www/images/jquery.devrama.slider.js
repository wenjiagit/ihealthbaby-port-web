...
(function($){
	var DrSlider = function(element, options){
		...
		this.options = {
			...
			navigationColor: '#D52B2F',
			navigationHoverColor: '#9F1F22',
	...
	DrSlider.prototype = {
		...
		_attachNavigation: function(){
			...
			for(var i =0; i < this.num_slides; i++)
				navigation_html += '<div class="nav-link index'+i+'" data-num="'+i+'">'+(i+1)+'</div>';
		...
		_prev_next_process: function($element, callback){
			...
			this._isLoadedImages($element.data('images'), function(arr_size){
				...
				if($element.data('has-main-image')){
					...
					$element.data('original', {'width': '100%', 'height': original_height});
				}
